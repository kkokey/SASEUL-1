...
class CommitManager
{
    ...
    public function Initialize()
    {
        ...
        $this->db->bulk->delete([]);
        $this->db->BulkWrite($this->namespacePrecommit . '.transactions');
    ...
    public function Precommit($transaction_chunks, $min_timestamp, $max_timestamp)
    {
        ...
        if ($this->db->bulk->count() > 0) {
            $namespace = $this->namespacePrecommit . '.transactions';
    ...
    public function CommitTransaction()
    {
        ...
        if ($this->db->bulk->count() > 0) {
            $this->db->BulkWrite($this->namespaceCommitted . '.transactions');
        }
    }
    ...
    public function CommitBlock()
    {
        ...
        if ($this->db->bulk->count() > 0) {
            $this->db->BulkWrite($this->namespaceCommitted . '.blocks');
        }
    }
    ...
    public function MakeTransactionChunk($chunks)
    {
        ...
        foreach ($chunks as $chunk) {
            $broadcast_chunk = Config::$block->getBroadcastChunks() . '/' . $chunk;
            $chunk_key = preg_replace('/\.json$/', '.key', $broadcast_chunk);
            $tx_chunk = Config::$block->getTransactions() . '/' . $transaction_dir . '/' . $blockhash . $timestamp . '.json';
