...
class Chunk
{
    public static function GetExpectStandardTimestamp($last_s_timestamp)
    {
        ...
        $max_s_timestamp = DateTime::Microtime() - Config::$block->getMicroIntervalOfChunk();
    ...
    public static function RemoveAPIChunk($s_timestamp)
    {
        ...
        foreach ($files as $file) {
            $filename = Config::$block->getApiChunks() . '/' . $file;
            unlink($filename);
        }
    }
    ...
    public static function RemoveBroadcastChunk($s_timestamp)
    {
        ...
        foreach ($files as $file) {
            $filename = Config::$block->getBroadcastChunks() . '/' . $file;
            unlink($filename);
        }
    }
    ...
    public static function GetChunkSignature($filename)
    {
        ...
        return Key::makeSignature($thash, Config::$node->getPrivateKey(), Config::$node->getPublicKey());
    }
    ...
    public static function ValidateContentsSignature($contents, $public_key, $signature)
    {
        ...
        return Key::isValidSignature($thash, $public_key, $signature);
    }
    ...
    {
        $subdir = Config::$block->getTransactions();
    ...
    public static function SaveBroadcastChunk($chunk)
    {
        ...
        $filename = Config::$block->getBroadcastChunks() . '/' . $name;
    ...
    public static function SaveAPIChunk($contents, $timestamp)
    {
        $filename = Config::$block->getApiChunks() . '/'
            . Config::$block::CHUNK_PREFIX . self::GetID($timestamp) . '.json';
